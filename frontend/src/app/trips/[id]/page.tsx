...
export default function TripDetailPage() {
  ...
  type ActiveTab = "itinerary" | "budget" | "hotels" | "weather" | "chat";
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      alert(
        e.response?.data?.message || "Generation failed. Check your API key.",
      );
    } finally {
      setGenerating(false);
    }
  ...
  const handleRegenerateDay = async (dayNumber: number) => {
    ...
    try {
      const { data } = await tripsAPI.regenerateDay(
        id,
        dayNumber,
        regenPrefs || "more variety",
      );
      setTrip((t) =>
        t
          ? {
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {generating && (
          <div className="card p-10 text-center mb-6 animate-pulse-slow">
            ...
            <p className="text-slate-400 text-sm mt-1">
              Building itinerary, budget, and hotel suggestions simultaneously
            </p>
          </div>
        )}
        ...
        {trip.isGenerated && (
          <>
                      ...
                      <div className="border-t border-slate-100 p-4">
                        <div className="space-y-3">
                          {day.activities.map((activity: Activity) => (
                            <div
                              key={activity._id}
                              className="flex items-start gap-3 group p-3 rounded-xl hover:bg-slate-50 transition-colors"
                            >
                              <div className="text-xs text-slate-400 w-16 flex-shrink-0 pt-0.5">
                                {activity.time}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  <span
                                    className={`badge text-xs ${activityTypeColors[activity.type] || activityTypeColors.other}`}
                                  >
                                    {activity.type}
                                  </span>
                                </div>
                              ...
                              <button
                                onClick={() =>
                                  handleRemoveActivity(day.day, activity._id!)
                                }
                        ...
                        <div className="flex gap-2 mt-4 pt-3 border-t border-slate-100">
                          <button
                            onClick={() => setShowAddActivity(day.day)}
                    ...
                    {(
                      ...
                      >
                        ...
                        <span className="font-medium text-slate-900">
                          ...
                          {trip.budget[key].toLocaleString()}
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="space-y-2">
                    {(
                      ...
                      const pct =
                        trip.budget.total > 0
                          ? Math.round(
                              (trip.budget[key] / trip.budget.total) * 100,
                            )
                          : 0;
                      return (
                        <div key={key}>
            ...
            {activeTab === "hotels" && (
              <div className="space-y-3 animate-fade-in">
                {["budget", "mid-range", "luxury"].map((cat) => (
                  <div key={cat}>
                    <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2 px-1 capitalize">
                      {cat}
                    </h3>
                    <div className="space-y-2">
                      {trip.hotels
                ...
                {!trip.weatherAdvice ? (
                  <div className="card p-10 text-center">
                    ...
                    <p className="text-slate-400 text-sm max-w-sm mx-auto mb-6">
                      Get AI-powered day-by-day weather forecasts with packing
                      tips, clothing recommendations, and activity impact alerts
                      tailored to {trip.destination}.
                    </p>
                    <button
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <h2 className="font-semibold text-slate-900">
                              Weather overview —{" "}
                              {trip.weatherAdvice.travelMonth}
                            </h2>
                          </div>
                          <p className="text-sm text-slate-600">
                            {trip.weatherAdvice.overview}
                          </p>
                        </div>
                        <button
                          onClick={handleGetWeather}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="card p-4">
                        ...
                        <ul className="space-y-1.5">
                          {(trip.weatherAdvice.generalPackingList || []).map(
                            (item, i) => (
                              <li
                                key={i}
                      ...
                      <div className="card p-4">
                        ...
                        <ul className="space-y-1.5">
                          {(
                            trip.weatherAdvice?.clothingRecommendations || []
                          ).map((item, i) => (
                            <li
                              key={i}
                              className="flex items-center gap-2 text-sm text-slate-600"
                        ...
                        {trip.weatherAdvice.healthTips.length > 0 && (
                          <div className="mt-3 pt-3 border-t border-slate-100">
                            ...
                            {(trip.weatherAdvice.healthTips || []).map(
                              (tip, i) => (
                                <p
                                  key={i}
                          ...
                          <div key={dayW.day} className="card p-4">
                            ...
                            <div className="grid grid-cols-3 gap-2 mb-3">
                              {[
                                ...
                                >
                                  <Icon
                                    className={`w-3.5 h-3.5 mx-auto mb-0.5 ${color}`}
                                  />
                                  <div className="text-xs text-slate-400">
                                    {label}
                                  </div>
            ...
            {activeTab === "chat" && (
              <div
                className="card flex flex-col animate-fade-in"
                style={{ height: "60vh" }}
              >
                  ...
                  {chatMessages.length === 0 && (
                    <div className="text-center py-8">
                      ...
                      <p className="text-slate-300 text-xs mt-1">
                        e.g. "What should I pack?" or "Suggest vegan restaurants
                        on Day 2"
                      </p>
                    </div>
                  )}
        ...
        {!trip.isGenerated && !generating && (
          <div className="card p-12 text-center">
            ...
            <p className="text-slate-400 mb-6 max-w-md mx-auto">
              Click &quot;Generate itinerary&quot; above and our AI will create
              a day-by-day plan, budget estimate, and hotel suggestions for{" "}
              {trip.destination}.
            </p>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="card p-6 w-full max-w-md animate-slide-up">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => handleRegenerateDay(showRegenModal)}
      ...
      {showAddActivity !== null && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="card p-6 w-full max-w-md animate-slide-up">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-slate-900">
                Add Activity to Day {showAddActivity}
              </h3>
              <button
